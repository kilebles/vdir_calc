...
async def create_post_callback(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        "<b><i>✏ Введите название поста</i></b>", parse_mode="HTML"
    )
...
async def delete_post_handler(
    callback: CallbackQuery, callback_data: DeletePostCallback
):
    ...
    if post.media_content:
        if post.media_content.startswith("AgAC"):
            media = InputMediaPhoto(
                media=post.media_content, caption=post_text, parse_mode="HTML"
            )
        else:
        ...
        await callback.message.edit_text(
            text=post_text, reply_markup=keyboard, parse_mode="HTML"
        )
...
async def edit_description_handler(
    callback: CallbackQuery, callback_data: EditDescriptionCallback, state: FSMContext
):
    post_id = callback_data.id
    ...
    try:
        ...
        await callback.message.answer(
            "<b>✒ Введите новое описание для поста:</b>", parse_mode="HTML"
        )
...
async def update_description_handler(message: Message, state: FSMContext):
    ...
    if post.media_content:
        ...
        await message.answer(text=post_text, reply_markup=keyboard, parse_mode="HTML")


# ^ Изменение медиа
@router.callback_query(EditMediaCallback.filter())
async def edit_media_handler(
    callback: CallbackQuery, callback_data: EditMediaCallback, state: FSMContext
):
    post_id = callback_data.id
    ...
    try:
        await callback.message.delete()
        await callback.message.answer(
            text="<b>🖼️ Отправьте новое фото</b>", parse_mode="HTML"
        )
    except Exception as e:
...
async def update_media_handler(message: Message, state: FSMContext):
    ...
    if message.photo:
        ...
        error_message = await message.answer(
            "<b>💢 Пожалуйста, отправьте фото</b>", parse_mode="HTML"
        )
        await asyncio.sleep(3)
        await error_message.delete()
        return
...
async def edit_time_handler(
    callback: CallbackQuery, callback_data: EditTimeCallback, state: FSMContext
):
    post_id = callback_data.id
    ...
    try:
        await callback.message.delete()
        await callback.message.answer(
            text="<b>🕜 Отправьте новое время</b>", parse_mode="HTML"
        )
    except Exception as e:
        await callback.answer("Ошибка при обновлении сообщения", show_alert=True)
...
async def update_time_handler(message: Message, state: FSMContext):
    ...
    try:
        ...
        error_message = await message.answer(
            "<b>💢 Произошла ошибка при обновлении времени.</b>", parse_mode="HTML"
        )
        await asyncio.sleep(3)
        await error_message.delete()
...
async def back_to_list_handler(
    callback: CallbackQuery, callback_data: BackToListCallback
):
    posts = await get_all_posts()
    keyboard = get_admin_keyboard(posts)
...
async def toggle_active_handler(
    callback: CallbackQuery, callback_data: ToggleActiveCallback
):
    post_id = callback_data.id
    post = await get_post_by_id(post_id)
